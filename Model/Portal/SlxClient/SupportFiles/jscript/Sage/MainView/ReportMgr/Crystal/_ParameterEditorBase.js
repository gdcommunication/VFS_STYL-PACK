    ...
    var _parameterEditorBase = declare('Sage.MainView.ReportMgr.Crystal._ParameterEditorBase', [_Widget, _Templated], {
        ...
        */
        ...
        initializeTransferBoxWidget: function (container, initializeList) {
            ...
            //check to see if there are any default values to be added to the selected values list
            if (initializeList) {
                dojoArray.forEach(this._initialIds, function (entry, i) {
                    self.transferBox.addSelectedItem(entry);
                });
            } else {
                //since the list was not initialized we need to add any default values to the base store and mark it as selected
                dojoArray.forEach(this._initialIds, function (entry, i) {
                    dojoArray.some(self._values, function (option, i) {
                        if (entry === option._id) {
                            var item = {
                                id: option._id,
                                value: option.value ? option.value : option.description,
                                displayValue: option.description,
                                __selected: true
                            };
                            self.transferBox.addNewItem(item, false);
                            return true;
                        }
                    });
                });
