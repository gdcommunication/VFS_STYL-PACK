    ...
    var groupListTasklet = declare('Sage.Groups.GroupListTasklet', [_Widget, _LocalStorageMixin], {
        ...
        _groupChangedAndLoaded: function () {
            ...
            if (this.grid.rowCount === 0) {
                console.log('_groupChangedAndLoaded aborted');
                return;
            }
        ...
        _createList: function () {
            ...
            if (this.grid) {
                ...
                //Fine tuning of count of records fetched from the server
                if (height >= 550) {
                    //We are in a large screen, load more records on a single sdata call
                    console.log('EGL: large screen detected');
                    grid.rowsPerPage = 35; //35 is the count of records that fit on a single page in Chrome at 1920x1080 resolution
                }
                else {
                    grid.rowsPerPage = 25;
                }
                //End fine tuning of records fetched from the server
        ...
        */
        _getCurrentGroupLayout: function () {
            ...
            if (!layout) {
                ...
                var url = "slxdata.ashx/slx/system/-/groups('" + this._getGroupID() + "')?select=layout/alias,layout/caption,layout/width,layout/fieldType,layout/format,layout/formatString,layout/propertyPath,layout/visible,layout/webLink,layout/dataPath&format=json";
        ...
        */
        _getFromSessionStorage: function (cacheKey) {
            ...
            if (cacheData !== null && typeof cacheData !== 'undefined') {
                console.log('_getFromSessionStorage: ' + cacheKey + ' found');
        ...
        */
        _getGridStructure: function () {
            ...
            if (this.mode === 'Standard') {
                ...
                if (!this._gridStructure || this.shouldRecreateGrid) {
                    var gridStructureData = groupsUtility.getGridStructure(layout, true);
                    this._gridStructure = gridStructureData.structure;
                    this._gridSelect = gridStructureData.select;
                }
        ...
        */
        _getHeight: function () {
            ...
            var gridHeight = taskPaneHeight - commonTasksHeight - 55;
        ...
        _connectToCommonTasksToggleEvent: function () {
            ...
            if (commonTasksTasklet) {
                dojo.connect(commonTasksTasklet, "toggle", function () {
                    //console.log("TaskPane_item_CommonTasksTasklet toggle event");;
